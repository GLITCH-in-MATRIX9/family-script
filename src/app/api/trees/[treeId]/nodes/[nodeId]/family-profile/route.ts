...
/**
 * GET /api/trees/[treeId]/nodes/[nodeId]/family-profile
 */
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const profile = await treeService.getFamilyProfile(
      treeId,
      session.user.id,
      nodeId
    );

    return successResponse(profile, "Family profile fetched successfully.");
  } catch (error) {
...
/**
 * PATCH /api/trees/[treeId]/nodes/[nodeId]/family-profile
 */
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const data = updateFamilyProfileSchema.parse(body);

    const profile = await treeService.updateFamilyProfile(
      treeId,
      session.user.id,
      nodeId,
      data
    ...
    return successResponse(profile, "Family profile updated successfully.");
  } catch (error) {
    return handleApiError(error);
  }
}